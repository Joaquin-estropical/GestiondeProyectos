...
import type { ProjectForm, ProjectFormItem, ChecklistTemplate } from '@/types';
...
function ProgressBar({ items }: { items: ProjectFormItem[] }) {
...
function CreateFormView({ projectId, currentUserId, onDone, onCancel }: {
  ...
  return (
    <div style={{ padding: '24px 32px', maxWidth: 640, overflowY: 'auto', height: '100%' }}>
      {/* Header */}
      ...
      <div style={{ marginBottom: 20 }}>
        <label style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.06em', color: 'var(--text-3)', display: 'block', marginBottom: 6 }}>
        ...
        <input
          ...
          style={{
            ...
            borderRadius: 6, padding: '9px 12px', fontSize: 14, color: 'var(--text-1)',
            outline: 'none',
          }}
        ...
        {source === 'template' && (
          <select
            ...
            style={{
              ...
              borderRadius: 6, padding: '8px 10px', fontSize: 13, color: 'var(--text-1)',
              outline: 'none',
            }}
          >
          ...
          {items.map((item, idx) => (
            <div key={idx} style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
              <input
                ...
                style={{
                  flex: 2, background: 'var(--surface-2)', border: '1px solid var(--border)',
            ...
            {catItems.map(item => (
              <div
                ...
                style={{
                  ...
                  border: `1px solid ${item.status === 'ok' ? 'rgba(20,184,166,.2)' : item.status === 'fail' ? 'rgba(239,68,68,.2)' : 'var(--border)'}`,
